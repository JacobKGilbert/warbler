...
def profile():
    ...
    form = UserEditForm()
    curr_user_username = g.user.username

    if form.validate_on_submit():
        try:
            user = User.authenticate(curr_user_username,
                                 form.password.data)
...
def homepage():
    ...
    if g.user:
        followed_ids = [f.id for f in g.user.following] + [g.user.id]
        likes = [msg.id for msg in g.user.likes]
        messages = (Message
                    .query
